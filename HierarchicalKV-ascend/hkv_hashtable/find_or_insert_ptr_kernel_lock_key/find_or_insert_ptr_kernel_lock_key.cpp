...
/*!
 * \file find_or_insert_ptr_kernel_lock_key.cpp
 * \brief find_or_insert_ptr_kernel_lock_key
 */

#include "./v35/find_or_insert_ptr_kernel_lock_key.h"
#include <cstdint>
...
extern "C" __global__ __aicore__ void find_or_insert_ptr_kernel_lock_key(
  ...
  DISPATCH_VALUE_SIZE(
      ...
      DISPATCH_EVICT_STRATEGY(
          evict_strategy,
          (Simt::VF_CALL<
              find_or_insert_ptr_kernel_lock_key_vf<uint64_t, DTYPE, uint64_t,
                                                    STRATEGY>>(
              Simt::Dim3{static_cast<uint32_t>(THREAD_NUM)}, buckets,
              buckets_size, capacity, bucket_capacity, dim, keys, value_ptrs,
              scores, key_ptrs, n, founds, global_epoch, total_thread_num,
              system_cycle, GetBlockIdx(), max_bucket_shift,
